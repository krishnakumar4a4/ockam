...
impl DefaultVault {
    ...
    fn hkdf_sha256_internal(&mut self,
        ...
        for attributes in output_attributes {
            let secret = match attributes.xtype {
                SecretKeyType::Buffer(size) => Ok(SecretKey::Buffer(okm[index..index+size].to_vec())),
...
mod tests {
    ...
    fn ec_diffie_hellman_p256() {
        ...
        let res = vault.ec_diffie_hellman_hkdf_sha256(sk_ctx_1, pk_2, salt, vec![output_attributes.clone()]);
        assert!(res.is_ok());
        let ss = res.unwrap();
        assert_eq!(ss.len(), 1);
        let ss = ss[0];
        ...
        let res = vault.ec_diffie_hellman_hkdf_sha256(sk_ctx_2, pk_1, salt, vec![output_attributes.clone()]);
        assert!(res.is_ok());
        let ss = res.unwrap();
        assert_eq!(ss.len(), 1);
        let ss = ss[0];
        ...
        let res = vault.ec_diffie_hellman_hkdf_sha256(sk_ctx_1, pk_1, salt, vec![output_attributes.clone()]);
        assert!(res.is_ok());
        let ss = res.unwrap();
        assert_eq!(ss.len(), 1);
        let ss = ss[0];
        ...
        let res = vault.ec_diffie_hellman_hkdf_sha256(sk_ctx_2, pk_2, salt, vec![output_attributes.clone()]);
        assert!(res.is_ok());
        let ss = res.unwrap();
        assert_eq!(ss.len(), 1);
        let ss = ss[0];
